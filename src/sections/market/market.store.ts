...
export const useMarketStore = defineStore('market', () => {
  ...
  const marketPriceHistory = ref([
    3168, 3226, 3224, 3222, 3201, 3222, 3171, 3196, 3188, 3190, 3196, 3201, 3222, 3218, 3157, 3218,
    3202, 3218, 3222, 3224, 3046, 3226, 3218, 3202, 3234, 3219, 3249, 3279, 3219, 3279, 3255, 3249,
    3236, 3185, 3201, 3249, 3230, 3243, 3255, 3230, 3231, 3253, 3244, 3237, 3244, 3253, 3279, 3264,
    3266, 3279, 3283, 3287, 3285, 3268, 3285, 3290, 3286, 3067, 3286, 3289, 3287, 3286, 3287, 3285,
    3286, 3285, 3034, 3287, 3289, 3290, 3291, 3258, 3291,
  ])
  ...
  function placeBuyOrder(quantity: number, price: number) {
    ...
    if (price >= askPrice.value) {
      ...
      // Fulfill order
      toast.success(`You have purchased ${quantity} Blue Pigment for ${formatter(actualPrice)}`)
  ...
  function simulateMarket() {
    ...
    for (let i = 0; i < actualTransactions; i++) {
      ...
      if (orderType === 'buy') {
        ...
        orderPrice = Math.round(
          gaussianRandom(currentPrice * (1 + drift * 2), currentPrice * standardDeviation),
        )
      }
